...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://cassiano:@localhost:5432/pgloader'
db = SQLAlchemy(app)
...
class AggregatedScoreByState(db.Model):
    ...
    def aggregated_scores_by_state_and_year_and_enem_subject_id(cls, state, year, enem_subject_id):
        return cls.query.filter_by(state=state, year=year, enem_subject_id=enem_subject_id.lower())
...
@app.route("/enem_subjects")
def enem_subjects_index():
    return jsonify([[es.id, es.name] for es in EnemSubject.query])

@app.route("/schools/<id>/aggregated_scores/<year>/<enem_subject_id>")
...
@app.route("/states/<state>/aggregated_scores/<year>/<enem_subject_id>")
...
@app.route("/cities/<city_id>/aggregated_scores/<year>/<enem_subject_id>")
...
@app.route("/schools/search/<city_id>/<term>")
def school_search(city_id, term):
    schools = School.search(city_id, term)
        
    return jsonify([[s.id, s.name] for s in schools])
